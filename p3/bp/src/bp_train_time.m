...
%
function [b w n_ages error] = bp_train_time(Train_set, Train_class, alpha, hidden_layer_n, max_ages, output_file)
  ...
  while (stop_condition == false)
    ...
    for i = 1 : n_samples
      ...
      %% Backpropagation %%
      % Output layer
      delta = (Train_class(i, :)' - y) .* @f_prime_identity(y);
      ...
      mse_basic_iter += sum((Train_class(i, :) - repmat(Train_set(i,n_attr),1,n_classes)).^2);
    ...
    if (output_file ~= -1)
      ...
      fprintf(output_file, "%f", mse_basic_iter / n_samples);
      fprintf(output_file, "\n");
    end
